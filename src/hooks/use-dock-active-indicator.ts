...
export function useDockActiveIndicator(activeKey: string | undefined) {
  ...
  function handleNavigate(href: string, keyboardInitiated: boolean) {
    keyboardNavigationRef.current = href !== activeKeyRef.current && keyboardInitiated
  }
  ...
  useLayoutEffect(() => () => clearIndicatorFrame(), [clearIndicatorFrame])
