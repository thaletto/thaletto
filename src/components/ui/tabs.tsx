...
import { SURFACE_BG, surfaceClasses } from '~/lib/design/surface-classes'
import { useSurface } from '~/lib/design/surface-context'
...
const TabsList = forwardRef<HTMLDivElement, TabsListProps>(
  ({ children, className, variant = 'segmented', ...props }, ref) => {
    ...
    const subtleIndicatorClass = SURFACE_BG[Math.round(Math.max(1, Math.min(8, substrate + 1)))]
      ...
      >
        ...
        >
          ...
          {selectedRect && (
            <div
              ...
              className={cn(
                ...
                variant === 'subtle' ? subtleIndicatorClass : surfaceClasses(indicatorLevel),
                'rounded-[20px]',
              )}
              style={{
          ...
          {hoverRect && !isHoveringSelected && selectedRect && (
            <div
              className={cn('absolute pointer-events-none bg-hover', 'rounded-[20px]')}
              style={{
                left: hoverRect.left,
...
const TabItem = forwardRef<HTMLButtonElement, TabItemProps>(
  ({ value, icon: Icon, label, _index = 0, className, ...props }, ref) => {
    ...
    const { registerTab, hoveredIndex, selectedValue, setOptimisticIdx, variant } = useTabsList()
