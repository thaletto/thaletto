...
function Select({
  ...
  const ctx = useMemo(() => ({ value: currentValue, open }), [currentValue, open])
...
const SelectTrigger = forwardRef<HTMLButtonElement, SelectTriggerProps>(
  ({ className, variant, size, icon: Icon, placeholder = 'Select…', error, ...props }, ref) => {
    return (
      <div className="flex flex-col gap-1">
        ...
        {error && <span className="pl-3 text-[14px] text-destructive">{error}</span>}
      </div>
    )
  },
)
...
const SelectContent = forwardRef<HTMLDivElement, SelectContentProps>(
  ({ className, children }, ref) => {
    ...
    const { activeIndex, setActiveIndex, itemRects, handlers, registerItem, measureItems } =
...
const SelectLabel = forwardRef<HTMLDivElement, HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div
      ref={ref}
      className={cn('px-2 py-1.5 text-[14px] text-muted-foreground', className)}
      {...props}
    />
  ),
)
