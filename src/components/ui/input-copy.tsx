...
const InputCopy = forwardRef<HTMLDivElement, InputCopyProps>(
  (
    ...
    const iconSwitch = (
      <span
        className={cn('flex items-center justify-center', status === 'error' && 'text-destructive')}
      >
        {statusIcon}
      </span>
    )
    ...
    const actionElement =
      variant === 'button' ? (
        ...
        >
          <span
            className={cn('flex items-center gap-1.5', status === 'error' && 'text-destructive')}
          >
            ...
            <span className="select-none inline-grid text-left">
              ...
              <span className="col-start-1 row-start-1">
                {status === 'error' ? 'Failed' : status === 'copied' ? 'Copied' : 'Copy'}
              </span>
            </span>
          </span>
        </span>
        ...
        <span
          className={cn(
            'shrink-0 px-1.5 py-2 transition-colors duration-150',
            'text-muted-foreground group-hover:text-foreground',
          )}
        >
          {iconSwitch}
    ...
    const valueElement = (
      <span
        ...
        style={{ fontVariationSettings: fontWeights.normal }}
      >
        <mark className="bg-transparent text-foreground transition-colors duration-150 group-hover:bg-[color:var(--active)] group-hover:text-foreground">
          {value}
        </mark>
      </span>
    )
