...
interface DialogContentProps extends ComponentPropsWithoutRef<typeof DialogPrimitive.Popup> {
  size?: 'sm' | 'lg'
}
    ...
    <DialogPrimitive.Portal>
      ...
      <DialogPrimitive.Popup
        ...
        className={cn(
          ...
          size === 'sm' ? 'w-[min(25rem,calc(100vw-2rem))]' : 'w-[min(34rem,calc(100vw-2rem))]',
          className,
        )}
        {...props}
      >
...
function DialogBody({ className, children }: DialogBodyProps) {
  ...
  return (
    <ScrollAreaY
      ...
      style={{ '--scroll-fade-surface': `var(--surface-${surface})` } as CSSProperties}
    >
...
function DialogClose({ variant = 'ghost', size = 'sm', children, ...props }: ButtonProps) {
  return (
