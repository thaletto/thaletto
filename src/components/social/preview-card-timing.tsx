...
export function PreviewCardTimingProvider({ children }: { children: React.ReactNode }) {
  ...
  return (
    <PreviewCardTimingContext.Provider value={shared}>
      ...
      <PreviewCard.Root handle={handle} onOpenChange={handleOpenChange}>
        {({ payload }) => (payload ? <PreviewSurface payload={payload} /> : null)}
      </PreviewCard.Root>
...
function StandalonePreviewCard({
  ...
  return (
    <>
      <PreviewTrigger {...props} payload={payload} handle={handle} delay={PREVIEW_OPEN_DELAY_MS} />
...
export function SitePreviewCard(props: SitePreviewCardProps) {
  ...
  return <PreviewTrigger {...props} payload={payload} handle={shared.handle} delay={shared.delay} />
}
