...
export async function Tweet({ slug, id }: { slug: string; id: string }) {
  ...
  const avatarFile = ['jpg', 'png'].find((ext) =>
    existsSync(path.join(dir, `tweet-${id}-avatar.${ext}`)),
  )
    ...
    <a className="tweet-card" href={tweet.url} target="_blank" rel="noreferrer">
      <span className="tweet-card-head">
        {avatarFile ? (
          ...
          // biome-ignore lint/performance/noImgElement: build-time data URI, see above
          <img src={fetched} alt="" width={40} height={40} className="tweet-card-avatar" />
        ) : (
      ...
      <span className="tweet-card-foot">
        ...
        {likes !== null && <span>· {likes.toLocaleString('en-US')} Likes</span>}
