...
import { useWebGpuCapability } from '~/components/visual/use-webgpu-capability'

const ProjectsBlueprintField = dynamic(
  () => import('./projects-blueprint-field').then((module) => module.ProjectsBlueprintField),
  { ssr: false },
)
...
function stageClassName(variant: StageVariant, className?: string) {
  return [`hidden-list-stage`, `${variant}-hidden-stage`, className].filter(Boolean).join(' ')
}
...
function HiddenListStage({
  ...
  const clearIntentTimer = useCallback(() => {
    if (intentTimer.current !== undefined) window.clearTimeout(intentTimer.current)
  ...
  const measureRow = useCallback(
    (row: HTMLElement) => {
      ...
      setGeometry({
        ...
        y: targetRect.top - rootRect.top + targetRect.height / 2 - WRITING_LANE_HEIGHT / 2,
        width: targetRect.width,
  ...
  const syncStage = useCallback(() => {
    ...
    const wantsMotion =
      pointerRow !== null && !reducedMotion.current && webGpuSupported.current !== false
  ...
  }, [checkWebGpu, clearExitTimer, config, setStageRow, webGpuSupported.current])
  ...
  return (
    ...
    >
        ...
        <span className="hidden-list-stage-field" aria-hidden>
          {variant === 'projects' ? (
            <>
              ...
              {shaderMounted && (
                <span className="hidden-list-stage-shader" data-list-shader-stage>
              ...
              <svg className="projects-registration-mark" viewBox="0 0 20 20" fill="none">
            ...
            <span className="writing-ink-current-target">
              {!shaderReady && (
                <svg className="writing-ink-static" viewBox="0 0 100 12" preserveAspectRatio="none">
      ...
      <div className={['hidden-list-stage-content', contentClassName].filter(Boolean).join(' ')}>
        {children}
      </div>
    </div>
  )
...
export function ProjectsBlueprintStage({
  ...
  return (
    <HiddenListStage variant="projects" className={className} contentClassName={contentClassName}>
      {children}
    </HiddenListStage>
  )
}
...
export function WritingInkStage({
  ...
  return (
    <HiddenListStage variant="writing" className={className} contentClassName={contentClassName}>
      {children}
    </HiddenListStage>
  )
}
