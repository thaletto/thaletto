...
import { useHiddenShaderField } from '~/components/visual/use-hidden-shader-field'
...
export function WritingInkField({
  ...
  const { handleReady, ink, ready } = useHiddenShaderField(onUnavailable, onReady)
  const stops = useMemo(
    () => [
  ...
  return (
    <span className="hidden-list-shader-shell" data-ready={ready ? 'true' : 'false'}>
      <Shader
